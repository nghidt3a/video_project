...
class MessyTransformation(BaseScene):
    def construct(self):
        title = self.title_banner("Hình tròn trong không gian bị ma trận tác động")
        ...
        vec1 = Arrow(ORIGIN, circle.point_at_angle(0), color=COLOR_U, buff=0, stroke_width=6)
        vec2 = Arrow(ORIGIN, circle.point_at_angle(PI * 0.78), color=COLOR_V, buff=0, stroke_width=6)

        self.play(FadeIn(title), Create(plane), Create(circle), GrowArrow(vec1), GrowArrow(vec2), run_time=1.2)
        self.wait(0.5)
        ...
        self.play(
            ...
            vec1.animate.put_start_and_end_on(ORIGIN, plane.c2p(A[0, 0] * 1.5, A[1, 0] * 1.5)),
            vec2.animate.put_start_and_end_on(ORIGIN, plane.c2p(A[0, 0] * -1.2 + A[0, 1] * 1.0, A[1, 0] * -1.2 + A[1, 1] * 1.0)),
            run_time=2.5,
        )
