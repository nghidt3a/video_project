...
class SVDIntroScene(BaseScene):
    def construct(self):
        ...
        result_ellipse = Ellipse(
            ...
            color=COLOR_TEXT, stroke_width=2,
        ).move_to(RIGHT * 3.5)
        label_s1 = MathTex(rf"\sigma_1 \approx {s[0]:.1f}", font_size=22)
        label_s2 = MathTex(rf"\sigma_2 \approx {s[1]:.1f}", font_size=22)
        label_s1.next_to(result_ellipse, RIGHT, buff=0.15)
        label_s2.next_to(result_ellipse, UP, buff=0.1).shift(RIGHT * 0.8)
        ...
        # VO cue: apply A → circle becomes ellipse
